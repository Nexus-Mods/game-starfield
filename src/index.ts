...
import {
  testSFSESupported,
  installSFSE,
} from "./installers/starfield-sfse-installer";
import {
  testASILoaderSupported,
  installASILoader,
  testASIModSupported,
  installASIMod,
} from "./installers/starfield-asi-installer";
...
function main(context: types.IExtensionContext) {
  ...
  context.registerTest(
    "starfield-loose-files-check",
    "gamemode-activated",
    () => testLooseFiles(context.api) as any,
  );
  context.registerTest(
    "starfield-deprecated-fomod-check",
    "gamemode-activated",
    () => Promise.resolve(testDeprecatedFomod(context.api)) as any,
  );

  context.registerInstaller(
    "starfield-sfse-installer",
    25,
    testSFSESupported as any,
    (files) => installSFSE(context.api, files) as any,
  );
  context.registerInstaller(
    "starfield-asi-mod-installer",
    20,
    testASIModSupported as any,
    (files) => installASIMod(context.api, files) as any,
  );
  context.registerInstaller(
    "starfield-asi-loader-installer",
    25,
    (files, gameId) =>
      testASILoaderSupported(context.api, files, gameId) as any,
  ...
  context.registerAction(
    ...
    (gameId?: string[]) =>
      isStarfield(context, gameId) && lootSortingAllowed(context.api),
  );
  ...
  context.optional.registerCollectionFeature(
    ...
    (gameId: string, collection: any, from: types.IMod, to: types.IMod) =>
      clone(context.api, gameId, collection, from, to),
    title,
    condition,
    StarfieldData,
  );
  ...
  context.once(() => {
    context.api.setStylesheet(
      "starfield",
      path.join(__dirname, "starfield.scss"),
    );
    context.api.events.on("gamemode-activated", () =>
      onGameModeActivated(context.api),
    );
    context.api.onAsync(
      "will-deploy",
      (profileId: string, deployment: types.IDeploymentManifest) =>
        onWillDeployEvent(context.api, profileId, deployment),
    );
    context.api.onAsync(
      "did-deploy",
      (profileId: string, deployment: types.IDeploymentManifest) =>
        onDidDeployEvent(context.api, profileId, deployment),
    );
    context.api.onAsync("will-purge", (profileId: string) =>
      onWillPurgeEvent(context.api, profileId),
    );
    context.api.onAsync("did-purge", (profileId: string) =>
      onDidPurgeEvent(context.api, profileId),
    );
  });
...
async function onDidDeployEvent(
  api: types.IExtensionApi,
  profileId: string,
  deployment: types.IDeploymentManifest,
): Promise<void> {
...
async function onWillPurgeEvent(
  api: types.IExtensionApi,
  profileId: string,
): Promise<void> {
  if (!isCorrectGame(api, profileId)) {
  ...
  return fs
    .copyAsync(pluginsPath, PLUGINS_BACKUP, { overwrite: true })
    .catch((err) => null);
}

async function onDidPurgeEvent(
  api: types.IExtensionApi,
  profileId: string,
): Promise<void> {
  if (!isCorrectGame(api, profileId)) {
...
async function onWillDeployEvent(
  api: types.IExtensionApi,
  profileId: any,
  deployment: types.IDeploymentManifest,
): Promise<void> {
  ...
  const exists = await fs
    ...
    .catch((err) => false);
  if (!exists) {
    const entries = (await walkPath(discovery.path)).filter(
      (entry) =>
        !entry.isDirectory &&
        path.basename(entry.filePath) === TARGET_ASI_LOADER_NAME,
    );
