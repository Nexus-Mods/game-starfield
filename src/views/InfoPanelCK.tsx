...
export function InfoPanelCK() {
  ...
  return (
    <>
      ...
      <p>
        {t(
          "LOOT is a volunteer based service which uses plugin metadata added by the community to arrange your load order in the most optimal way - it's possible for certain plugin metadata to be unavailable (especially if it's a new mod) which can result in incorrect sorting.",
        )}
      </p>
    </>
  );
...
function displayLootInstruction(api: types.IExtensionApi, t: any) {
  return lootSortingAllowed(api) ? (
    <li>
      {t(
        'Press the "Sort via LOOT" button to sort your plugins based on the Starfield masterlist.',
      )}
    </li>
  ) : null;
}
