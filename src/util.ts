...
import {
  ...
  LOCAL_APP_DATA,
  GAME_ID,
  MY_GAMES_DATA_WARNING,
  JUNCTION_NOTIFICATION_ID,
  PLUGINS_BACKUP,
  XBOX_APP_X_MANIFEST,
...
export async function mergeFolderContents(
  source: string,
  destination: string,
  overwrite = false,
): Promise<void> {
  ...
  for (const sourceFile of sourceDir) {
    ...
    const stats: fs.Stats = await fs
      .statAsync(sourceFilePath)
    ...
    const destinationMatch = destinationDir.find(
      (f) => f.toLowerCase() === sourceFile.toLowerCase(),
    );
    if (stats.isDirectory()) {
      // Is there already a subfolder? If so, we need to merge the contents
      if (destinationMatch !== undefined) {
        await mergeFolderContents(
          sourceFilePath,
          path.join(destination, destinationMatch),
          overwrite,
        );
      } else {
        await fs.copyAsync(sourceFilePath, path.join(destination, sourceFile), {
          overwrite: true,
        });
      }
      continue;
      ...
      if (!destinationMatch || overwrite) {
        ...
        log("debug", "Did not copy file as it already exists", {
          source: sourceFilePath,
          destination: path.join(destination, destinationMatch),
        });
        continue;
      }
    } else
      return log(
        "warn",
        "Did not remove file as it is neither a file nor folder",
        sourceFilePath,
      );
  }
...
export async function nuclearPurge(
  ...
  const manifests: { [filePath: string]: types.IDeploymentManifest } =
    await entries.reduce(async (accumP, m) => {
...
export async function purgeDeployedFiles(
  basePath: string,
  files: types.IDeployedFile[],
): Promise<void> {
  for (const file of files) {
    ...
    await fs
      .statAsync(fullPath)
      .then((stats) =>
        stats.mtime.getTime() - file.time < 1000
...
export async function requiresPluginEnabler(
  api: types.IExtensionApi,
): Promise<boolean> {
  try {
    const gameVersion = await getGameVersionAsync(api);
    return semver.satisfies(
      semver.coerce(gameVersion).version,
      CONSTRAINT_PLUGIN_ENABLER,
    );
  } catch (err) {
...
export function getGameVersionSync(api: types.IExtensionApi): string {
  const state = api.getState();
  const gameVersion = util.getSafe(
    state,
    ["persistent", "gameMode", "versions", GAME_ID],
    "0.0.0",
  );
  return semver.coerce(gameVersion).raw;
}

export async function getGameVersionAsync(
  api: types.IExtensionApi,
): Promise<string> {
  const state = api.getState();
...
export async function getExtensionVersion() {
  const infoFile = JSON.parse(
    await fs.readFileAsync(path.join(__dirname, "info.json")),
  );
...
export async function doesModRequireMigration(
  modPath: string,
): Promise<boolean> {
  const dataPath = path.join(modPath, "Data");
...
export function dismissNotifications(api: types.IExtensionApi) {
  ...
  const notificationIds = [
    "starfield-junction-activity",
    MY_GAMES_DATA_WARNING,
    JUNCTION_NOTIFICATION_ID,
    INSTALLING_REQUIREMENTS_NOTIFICATION_ID,
    "starfield-update-notif-0.5.0",
    "starfield-update-notif-0.6.0",
    "starfield-update-notif-0.7.0",
...
export function deepMerge(lhs: any, rhs: any): any {
  ...
  for (const key of Object.keys(lhs).concat(Object.keys(rhs))) {
    ...
    result[key] =
      typeof lhs[key] === "object" && typeof rhs[key] === "object"
...
export function getEnabledMods(
  ...
  const predicate = (mod: types.IMod) =>
    !!modType ? isEnabled(mod.id) && mod.type === modType : isEnabled(mod.id);
...
export async function findModByFile(
  ...
  for (const mod of mods) {
    ...
    if (
      files.some(
        (file) =>
          path.basename(file.filePath).toLowerCase() ===
...
export async function linkAsiLoader(
  api: types.IExtensionApi,
  lhs: string,
  rhs: string,
): Promise<void> {
  ...
  const state = api.getState();
  const pluginEnabler = util.getSafe(
    state,
  ...
  const exists = await fs
    .statAsync(asiLoaderLhs)
    .then(() => true)
    .catch(() => false);
  if (exists) {
    await fs
      .linkAsync(asiLoaderLhs, asiLoaderRhs)
      .catch((err) => Promise.resolve());
...
export async function serializePluginsFile(
  api: types.IExtensionApi,
  plugins: types.ILoadOrderEntry[],
): Promise<void> {
  const nativePlugins = await resolveNativePlugins(api);
  // Blueprint plugins (Starfield) are force-loaded by the game and managed
  // by the engine itself. Writing them to plugins.txt causes the game to
  // strip them on launch, producing the "my plugins are disabled" UX bug
  // described in APP-263. Strip them here alongside natives.
  const blueprintPlugins = await resolveBlueprintPlugins(api);
  const data: string[] = plugins.reduce((acc, plugin) => {
    ...
    if (nativePlugins.includes(nameLower)) {
      return acc;
    }
    if (blueprintPlugins.has(nameLower)) {
      return acc;
    }
  ...
  return fs
    .writeFileAsync(pluginsFile, data.filter((plug) => !!plug).join("\n"), {
      encoding: "utf8",
    })
...
export async function deserializePluginsFile(): Promise<string[]> {
  try {
    const targetFile = await fs
      .statAsync(PLUGINS_BACKUP)
      .then(() => PLUGINS_BACKUP)
      .catch(() => PLUGINS_TXT);
    const data = await fs.readFileAsync(targetFile, "utf8");
    const lines = data.split("\n").filter((line) => line.trim().length > 0);
    return Array.from(new Set(lines.map((line) => line.toLowerCase())));
  } catch (err) {
    return [];
  }
}

export function setPluginManagementEnabled(
  api: types.IExtensionApi,
  enabled: boolean,
) {
  const state = api.getState();
...
export function getManagementType(
  api: types.IExtensionApi,
): LoadOrderManagementType {
  const state = api.store.getState();
  const profileId = selectors.lastActiveProfileForGame(state, GAME_ID);
  return util.getSafe(
    state,
    ["settings", GAME_ID, "loadOrderManagementType", profileId],
    "dnd",
  );
}
...
export const lootSortingAllowed = (api: types.IExtensionApi) => {
  const state = api.getState();
  const appVersion = DEBUG_ENABLED
    ? DEBUG_APP_VERSION
    : util.getSafe(state, ["app", "appVersion"], "0.0.1");
  return (
    appVersion === "0.0.1" ||
    semver.satisfies(semver.coerce(appVersion), CONSTRAINT_LOOT_FUNCTIONALITY)
  );
};

export const resolvePluginsFilePath = async (
  api: types.IExtensionApi,
): Promise<string> => {
...
/**
 * Walk the Data folder and return the set of lowercase basenames whose TES4
 * header has the Starfield Blueprint flag set. Detection is delegated to the
 * gamebryo-plugin-management extension via its `isBlueprintPlugin` API — this
 * avoids taking a direct native-addon (esptk) dependency on the starfield
 * extension side.
 *
 * Returns an empty set when:
 *   - the game isn't discovered,
 *   - gamebryo-plugin-management isn't loaded / the API isn't registered, or
 *   - the Data folder can't be read.
 *
 * Callers should treat "Blueprint" as a filter, not a mode — if the set is
 * empty, just behave as before (don't special-case anything).
 */
export const resolveBlueprintPlugins = async (
  api: types.IExtensionApi,
): Promise<Set<string>> => {
  const result = new Set<string>();
  const isBlueprint = (api as any).ext?.isBlueprintPlugin;
  if (typeof isBlueprint !== "function") {
    // gamebryo-plugin-management either isn't loaded or predates this API
    return result;
  }
  const state = api.getState();
  const discovery = selectors.discoveryByGame(state, GAME_ID);
  ...
  try {
    dirContents = await fs.readdirAsync(dataPath);
  } catch (err) {
    return result;
  }
  for (const file of dirContents) {
    if (!DATA_PLUGINS.includes(path.extname(file).toLowerCase())) {
      continue;
    }
    try {
      ...
      // API-side failure (parse error etc.) — treat as not-Blueprint so we
      // don't silently skip the plugin on writes or demote it in the list
      log("warn", "isBlueprintPlugin API call failed", {
        file,
...
export const resolveNativePlugins = async (
  api: types.IExtensionApi,
): Promise<string[]> => {
  const state = api.getState();
  const discovery = selectors.discoveryByGame(state, GAME_ID);
  const cccFilePath = PLUGINS_CCC_PATTERN.replace(
    "{{prefix}}",
    discovery?.path || "",
  );
  const dataPath = path.join(discovery.path, "Data");
...
export async function lootSort(api: types.IExtensionApi) {
  {
    ...
    const onSortCallback = async (err: Error, result: string[]) => {
      ...
      const filtered = result.filter(
        (plugin) => !nativePlugins.includes(plugin.toLowerCase()),
      );
      if (filtered.length === 0) {
        api.showErrorNotification(
          "No plugins to sort",
          "LOOT did not find any plugins to sort.",
        );
        return;
      }
    ...
    if (api.ext.lootSortAsync !== undefined) {
      ...
      fs.readdirAsync(dataPath)
        .then((contents) => {
          const pluginFilePaths = contents.reduce((accum, p) => {
            DATA_PLUGINS.includes(path.extname(p)) &&
              accum.push(path.join(dataPath, p));
            return accum;
          }, []);
        ...
        .catch((err) => {
          ...
          api.dismissNotification("starfield-fblo-loot-sorting");
          api.showErrorNotification(
            "Could not read the data folder to sort plugins.",
            err,
          );
          return Promise.resolve();
        });
      ...
      api.showErrorNotification(
        "LOOT sort API extension is unavailable",
        "Please ensure the Gamebryo Plugins Management extension is enabled",
      );
      api.dismissNotification("starfield-fblo-loot-sorting");
    }
    return Promise.resolve();
...
export async function switchToLoot(api: types.IExtensionApi) {
  const state = api.getState();
  const profileId = selectors.lastActiveProfileForGame(state, GAME_ID);
  const loadOrder: types.ILoadOrderEntry[] = util.getSafe(
    state,
    ["persistent", "loadOrder", profileId],
    [],
  );
  const nativePlugins = await resolveNativePlugins(api);
  const enabled = loadOrder
    .filter((l) => l.enabled && !nativePlugins.includes(l.name.toLowerCase()))
    .map((l) => l.name);
  try {
    await deploy(api);
